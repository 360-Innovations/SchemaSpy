...
public class Dot
{
	...
	private Dot()
	{
		...
		try
		{
			...
			else
			{
				...
				{
					System.err.println();
					System.err.println("Invalid dot configuration detected.  '" + getDisplayableCommand(dotCommand) + "' returned:");
	...
	/**
	 * Set the image format to generate. Defaults to <code>png</code>. See <a
	 * href='http://www.graphviz.org/doc/info/output.html'>http://www.graphviz.org/doc/info/output.html</a> for valid formats.
	 * 
	 * @param format
	...
	/**
	 * Returns true if the installed dot requires specifying :gd as a renderer. This was added when Win 2.15 came out because it defaulted to Cairo, which
	 * produces better quality output, but at a significant speed and size penalty.
	 * <p>
	 * 
	 * The intent of this property is to determine if it's ok to tack ":gd" to the format specifier. Earlier versions didn't require it and didn't know about
	 * the option.
	 * 
	 * @return
	 */
	...
	/**
	 * Set the renderer to use for the -Tformat[:renderer[:formatter]] dot option as specified at <a href='http://www.graphviz.org/doc/info/command.html'>
	 * http://www.graphviz.org/doc/info/command.html</a> where "format" is specified by {@link #setFormat(String)}
	 * <p>
	...
	{
		return renderer != null && supportsRenderer(renderer) ? renderer : (requiresGdRenderer() ? ":gd" : "");
	}

	/**
	 * If <code>true</code> then generate output of "higher quality" than the default ("lower quality"). Note that the default is intended to be
	 * "lower quality", but various installations of Graphviz may have have different abilities. That is, some might not have the "lower quality" libraries and
	 * others might not have the "higher quality" libraries.
	 */
	...
	/**
	 * Returns <code>true</code> if the specified renderer is supported. See {@link #setRenderer(String)} for renderer details.
	 * 
	 * @param renderer
	 * @return
	 */
